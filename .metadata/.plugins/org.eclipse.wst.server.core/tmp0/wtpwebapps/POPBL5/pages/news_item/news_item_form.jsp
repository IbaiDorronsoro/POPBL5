...
<c:choose>
	<c:when test="${empty requestScope.newsItem}">
		...
		<c:set var="action" scope="page" value="${basePath}/news/create"/>
		<c:set var="title" scope="page" value=""/>
		<c:set var="body" scope="page" value=""/>
	</c:when>
	<c:otherwise>
		<c:set var="pageTitle" scope="request" value="editUser"/>
		<c:set var="action" scope="page" value="${basePath}/news/${requestScope.newsItem.newsItemId}/edit"/>
	...
	<form class="card" action="${action}" method="post">
		...
		<div class="card-body">
			<label>
				...
				<input type="text" name="title" value="${title}" placeholder="<fmt:message key="title"/>"/>
			</label>
			<label>
				<fmt:message key="body"/>:
				 <textarea rows="4" cols="50" name="body" placeholder="<fmt:message key="body"/>"><c:out value="${requestScope.newsItem.body}"/></textarea>
			</label>
